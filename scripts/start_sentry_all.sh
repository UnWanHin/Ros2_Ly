...
REAL_CONFIG_REL="scripts/config/auto_aim_config_competition.yaml"
...
force_real_machine_config_file() {
  local real_config_abs="${ROOT_DIR}/${REAL_CONFIG_REL}"
  if [[ ! -f "${real_config_abs}" ]]; then
    echo "[ERROR] Real-machine config file not found: ${real_config_abs}" >&2
    exit 2
  fi

  if has_launch_arg_key "config_file"; then
    echo "[WARN] config_file override detected: $(launch_arg_value config_file || true)" >&2
    echo "[WARN] forcing real-machine config: ${real_config_abs}" >&2
    strip_launch_arg_key "config_file"
  fi

  LAUNCH_ARGS=("config_file:=${real_config_abs}" "${LAUNCH_ARGS[@]}")
}
...
echo "[INFO] Effective config_file (forced real-machine): $(launch_arg_value config_file || true)" >&2
